...
class LDA(object):
    ...
    def fit(self, x, y):
        ...
        if self._method == "twostage":
            ...
            raise NameError(
                'LDA solver not implemented, chose either "eig", "svd", "twostage" or "qrsvd"'
            )
    ...
    def fit_eig(self, x, y):
        # Class variances
        s_w = (
            np.dot((x[y == -1, :] - self._mu_n).T, (x[y == -1, :] - self._mu_n))
            / self._n
        )
        s_w += (
            np.dot((x[y == 1, :] - self._mu_p).T, (x[y == 1, :] - self._mu_p)) / self._n
        )
        # Within class variances
        s_b = (
            self._n_n
            * np.outer((self._mu_n - self._mu), (self._mu_n - self._mu))
            / self._n
        )
        s_b += (
            self._n_p
            * np.outer((self._mu_p - self._mu), (self._mu_p - self._mu))
            / self._n
        )
        # Total variance
        # s_t = np.dot((x-self._mu).T, (x-self._mu))/self._n
        # Outputs
        self._scalings, self._transform_matrix = np.linalg.eig(
            np.dot(np.linalg.inv(s_w), s_b)
        )
    ...
    def fit_svd(self, x, y):
        ...
        hb = (
            ...
            * np.reshape(
                (
                    np.sqrt(self._n_n) * (self._mu_n - self._mu),
                    np.sqrt(self._n_p) * (self._mu_p - self._mu),
                ),
                (2, self._m),
            ).T
        )
    ...
    def fit_srda(self, x, y):
        ...
        e2 = (
            y2
            - np.dot(y2, e0) / np.dot(e0, e0) * e0
            - np.dot(y2, e1) / np.dot(e1, e1) * e1
        )
        xb = x.copy()
        xb[y == 1, :] -= self._mu_p
        xb[y == -1, :] -= self._mu_n
        a = np.dot(
            np.dot(np.linalg.inv((np.dot(xb.T, xb))), xb.T),
            np.array([e0, e1, e2]).T[:, : self._od],
        )

...
class OSVM(object):
    ...
    def update(self, x, y):
        ...
        if y * np.sum(self._kt*self._alpha*self._oca_l) < 1:
            ...
            if self._oca_l.ndim != 0:
                ...
                self._alpha[self._oca_l == 1] = self._alpha[self._oca_l == 1] * sa / 2 / np.sum(self._alpha[self._oca_l == 1])
                self._alpha[self._oca_l == -1] = self._alpha[self._oca_l == -1] * sa / 2 / np.sum(self._alpha[self._oca_l == -1])
    ...
    def predict(self, xp):
        ...
        # Single prediction
        if xp.ndim == 1:
            yp = np.sum(self._alpha * self._oca_w * self._oca_l * self._kernel(self._oca_c, xp))
            return yp
        else:  # n-predictions.
            n = np.size(xp, axis=0)
            yp = np.zeros(n)
    ...
    def visual(self, grid_size=100, levels=10, plot_alpha=False, plot_kernel=False, plot_3=False):
        ...
        if plot_alpha:
            if not self._amem.any():
                print('Not plotting Lagrangian convergence, history not saved in optimization.')
